...
export default class BusinessAnswerView extends Component<Props>{
                    ...
                    <View style={styles.itemClickStyle}>
                        <View style={{ flexDirection: 'row', marginBottom: 5 }}>
                            <View style={{ backgroundColor: 'rgba(128,128,128, 0.4)', borderRadius: 11, paddingHorizontal: 5, paddingVertical: 5 }}>
                                ...
                                <Text style={{ fontSize: 10, color: '#F5F5F5', lineHeight: 13 }}>{"\"" + item.value + "\""}</Text>
                            </View>
                        </View>
                    </View>
                </TouchableOpacity>
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'nowrap', alignItems: 'baseline' }}>
                        ...
                        <Text numberOfLines={1} style={{ fontSize: 13, color: 'white', left: 20, flexWrap: 'wrap', width: 250 }}>{this.state.answerTitle}</Text>
                            ...
                            <ScrollView
                                ...
                                <View style={{ flex: 1 }}>
                                    <View style={{ height: this.state.webViewHeight, backgroundColor: 'transparent' }}>
                                        {this.getWebView()}
                                    </View>
                ...
                {this.state.showVideo ?
                    <Modal
                        ...
                        <View style={{
                            ...
                            height: "100%",
                            width: "100%",
                        }} ><VideoPlayScreen videoUrl={this.state.videoUrl} ref="videoPlayScreen" hideVideo={this.hideVideo} /></View></Modal> : null
    ...
    getWebView() {
        ...
        return <ScrollView style={{ backgroundColor: 'transparent', width: "100%", height: this.state.webViewHeight }}>
    ...
    getWebView2() {
        ...
        return <WebView
            ...
            style={{ backgroundColor: 'transparent', height: this.state.webViewHeight, width: "100%", marginBottom: 60 }}
            javaScriptEnabled={true}
            originWhitelist={['*']}
            ...
            onMessage={({ nativeEvent: { data } }) => {
                ...
                //http开头的是连接, 不是视频就是图片了
                    ...
                    console.log("data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data data ");
    ...
    requestAnswerData(questionStr) {
        ...
        //     },
        //     body: JSON.stringify(obj), // 将数据转换为JSON字符串作为请求体
        // };
        this.myLog("猎户发送的参数990: " + JSON.stringify(mBody));
        fetch(mUrl,{
            method: "POST",
            headers: {
                'Content-Type': 'application/json'
        ...
        // });
            ...
            .then((json) => {
                this.myLog(JSON.stringify(json));
                // console.log("\n");
                ...
                // 检查是不是跳转经办界面 || 检查是不是跳转到评价界面
                let aaaaa = this.nextBizName(jsonData.answers[0].plainText);
                this.myLog('>>>>>>>>>>>>> aaaaa <<<<<<<<<<<<<<');
                this.myLog(aaaaa);

                if (aaaaa != '') {
                    this.myLog('>>>>>>>>>>>>> aaaaa <<<<<<<<<<<<<< 进来了');
                    this.myJump(aaaaa, jsonData.answers[0].plainText);
                }
            ...
            .catch((error) => {
                ...
                this.myLog('requestAnswerData fetch error:' + error);
            });
    ...
    getProfessionalNoun(questionStr) {
        ...
        this.myLog("getProfessionalNoun"+ mUrl);
        fetch(mUrl)
            .then((response) => response.json())
            .then((json) => {
